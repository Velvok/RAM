...
-- Agregar assigned_inventory_id a cut_orders
ALTER TABLE cut_orders 
ADD COLUMN IF NOT EXISTS assigned_inventory_id UUID REFERENCES inventory(id) ON DELETE SET NULL;

-- Agregar assigned_inventory_id a preparation_items
ALTER TABLE preparation_items 
ADD COLUMN IF NOT EXISTS assigned_inventory_id UUID REFERENCES inventory(id) ON DELETE SET NULL;
...
COMMENT ON COLUMN cut_orders.assigned_inventory_id IS 'Inventario asignado para consumir stock reservado al entregar';
COMMENT ON COLUMN preparation_items.assigned_inventory_id IS 'Inventario asignado para consumir stock reservado al entregar';
