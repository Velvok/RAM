...
export default function PlantaPedidoDetallePage() {
  ...
  const completedClass = 'border-green-500 bg-green-900/10'
  const expandedClass = 'border-blue-500 bg-slate-800'
  const defaultClass = 'border-slate-600 bg-slate-800/50'
        ...
        <div className="space-y-4">
          {pedido.cut_orders && pedido.cut_orders.length > 0 ? (
            pedido.cut_orders.map((cutOrder: any) => {
              ...
              const cardClass = isCompleted ? completedClass : isExpanded ? expandedClass : defaultClass

              return (
                <div
                  key={cutOrder.id}
                  className={'rounded-lg border-2 overflow-hidden transition-all ' + cardClass}
                >
                  <div 
                    className={'p-4 transition-colors' + (!isCompleted ? ' cursor-pointer hover:bg-slate-700/30' : '')}
