      ...
      <div className="bg-white rounded-lg shadow p-6 mb-6">
        ...
        <div className="mb-6">
          ...
          <div className="flex flex-wrap gap-3">
            {filters.map((filter) => {
              const statusInfo = getStatusInfo(filter.id === 'todos' ? 'nuevo' : filter.id)
              return (
                <button
                  key={filter.id}
                  onClick={() => handleFilterChange(filter.id)}
                  className={`inline-flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 transform hover:scale-105 ${
                    statusFilter === filter.id
                      ? `${statusInfo.bgColor} ${statusInfo.textColor} border-2 ${statusInfo.borderColor} shadow-md`
                      : 'bg-white border-2 border-slate-200 text-slate-700 hover:border-slate-300 hover:shadow-sm'
                  }`}
                >
                  <div className={`w-2 h-2 rounded-full ${
                    filter.id === 'todos' ? 'bg-slate-400' :
                    filter.id === 'nuevo' ? 'bg-slate-500' :
                    filter.id === 'aprobado' ? 'bg-yellow-500' :
                    filter.id === 'en_corte' ? 'bg-blue-500' :
                    filter.id === 'finalizado' ? 'bg-green-500' :
                    filter.id === 'entregado' ? 'bg-slate-800' : 'bg-slate-400'
                  }`} />
